...
# 前面的话
## 函数对于任何一门语言来说都是核心的概念。通过函数可以封装任意多条语句，而且可以在任何地方，任何时候调用执行。在javascript世界里，函数即就是对象，程序可以随意操控它们，函数可以嵌套在其他函数中定义，这样它们就可以访问它们被定义时所处作用域中的任何变量，它（函数）给javascript带来了非常强劲的编程能力。
